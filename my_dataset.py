...
def get_train_data_loader(batch_size=64, num_workers=8, pin_memory=True):
    """
    获取训练数据加载器
    ...
    return DataLoader(
        ...
        persistent_workers=True if num_workers > 0 else False,  # 保持workers活跃
        prefetch_factor=2 if num_workers > 0 else None  # 预加载2个batch
    )

def get_test_data_loader(batch_size=64, num_workers=4, pin_memory=True):
    """
    获取测试数据加载器
